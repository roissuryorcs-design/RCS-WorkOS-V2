...
export default function BoardTable({
  items,
  setItems, // 🔥 tambahkan props ini
  groups: externalGroups,
  setGroups: externalSetGroups, // 🔥 tambahkan props ini
  ...
  const {
    ...
    handleDragStart,
    handleDragEnd,
  } = useDragDrop(items, setItems, groups, setGroups);
  ...
  useEffect(() => {
    if (externalSetGroups) {
      externalSetGroups(groups);
    }
  }, [groups, externalSetGroups]);
  ...
  // ============================================================
  // 🔥 GROUP IDs UNTUK SORTABLE CONTEXT
  // ============================================================
  const groupIds = groups.map(g => `group-${g}`);
          ...
          <div className="board-scroll-content">
            <SortableContext items={groupIds} strategy={verticalListSortingStrategy}>
              {groups.map((groupName) => {
                ...
                return (
                  <DraggableGroup key={`group-${groupName}`} id={`group-${groupName}`}>
                    <div 
          ...
          <div className="drag-overlay">
            {activeId.toString().startsWith('group-') ? (
              <div className="group-overlay">
                Moving: {activeId.toString().replace('group-', '')}
              </div>
            ) : (
              <div className="item-overlay">Moving item...</div>
            )}
          </div>
        ) : null}
      </DragOverlay>
